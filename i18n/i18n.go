...
const (
	LangZh    = "zh"
	LangZhCN  = "zh-CN" // 兼容别名（go-i18n 文件名）
	LangZhTW  = "zh-TW"
	LangEn      = "en"
	LangJa      = "ja"
	LangRu      = "ru"
	...
	LangVi      = "vi"
	LangAr      = "ar"
	DefaultLang = LangZh // Fallback to Chinese if language not supported
)
...
func Init() error {
	...
	initOnce.Do(func() {
		...
		// Pre-create localizers for supported languages (zh-TW/en have YAML; others fall back to en)
		localizers[LangZh] = i18n.NewLocalizer(bundle, LangZh, LangZhCN)
...
// messageLoaderFunc loads a user-facing message override from the database-backed
// i18n_messages table. Set by the model package to avoid an import cycle.
var messageLoaderFunc func(key, locale string) (string, bool)

// SetMessageLoader registers the DB-backed message loader. Once set, Msg() resolves
// user-facing strings via the i18n_messages table so translations can be edited at
// runtime without redeployment (配置即数据).
func SetMessageLoader(loader func(key, locale string) (string, bool)) {
	messageLoaderFunc = loader
}

// Msg returns the localized user-facing message for the current request locale.
// The source text (usually Chinese) is used as the translation key in the
// i18n_messages table; if no override exists for the locale, the source text is
// returned (always readable, zero regression).
func Msg(c *gin.Context, src string) string {
	if messageLoaderFunc != nil {
		if v, ok := messageLoaderFunc(src, GetLangFromContext(c)); ok && v != "" {
			return v
		}
	}
	return src
...
func SupportedLanguages() []string {
	return []string{LangZh, LangZhTW, LangEn, LangJa, LangRu, LangFr, LangVi, LangAr}
}
