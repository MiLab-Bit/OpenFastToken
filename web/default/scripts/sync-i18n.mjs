...
const LOCALES_DIR = path.resolve('src/l10n/locales')
...
// CJK (Chinese/Kanji) character range. Used to detect source-language text
// that leaked into a locale that should not contain CJK at all.
const CJK_RE = /[一-鿿]/
// Locales whose own script legitimately contains CJK (so a CJK char in their
// value is NOT a sign of an untranslated string).
...
function isLikelyUntranslated({ locale, baseValue, value }) {
  ...
  // Skip short tokens / acronyms / ids / urls / code literals
  ...
  // A value that still contains CJK characters in a locale whose script does
  // NOT use CJK (Arabic, Russian, French, Vietnamese, ...) almost certainly
  // means the Chinese source was never translated. This is the dominant case
  // for `ar` (its JSON is ~93% Chinese), which the old equality-with-EN check
  // missed entirely because the untranslated value equals ZH, not EN.
  if (!CJK_LOCALES.has(locale) && CJK_RE.test(value)) return true

  // Equality-with-base heuristics for the remaining cases.
...
async function main() {
  ...
  // Pick the base locale. Prefer the canonical source (FALLBACK_COMPARE_LOCALE,
  // i.e. `en`): ordering and missing/extras must be measured against the
  // reference translation, NOT against a locale that merely happens to carry
  // many keys (e.g. `ar`, whose file is ~93% untranslated Chinese duplicates
  // and would otherwise be wrongly chosen as the "richest" baseline).
  const candidates = Object.keys(parsedByLocale).map((locale) => {
    const json = parsedByLocale[locale]
    const trans = json?.translation ?? {}
    return { locale, score: countLeafKeys(trans) }
  })
  const baseLocale =
    candidates.find((c) => c.locale === FALLBACK_COMPARE_LOCALE)?.locale ??
    candidates.sort((a, b) => b.score - a.score || a.locale.localeCompare(b.locale))[0]?.locale
