...
export function formatNumber(value: number | null | undefined): string {
  ...
  return Intl.NumberFormat(getIntlLocale(), { maximumFractionDigits: 2 }).format(
    value as number
  )
}
...
export function formatTimestampRelative(
  ...
  const ts = unit === 'seconds' ? timestamp * 1000 : timestamp;
  const now = Date.now();
  const diff = now - ts;
  const seconds = Math.floor(diff / 1000);
  if (seconds < 0) return formatTimestamp(timestamp);
  if (seconds < 60) return i18n.t('time.just_now');
  const minutes = Math.floor(seconds / 60);
  if (minutes < 60) return i18n.t('time.ago_minutes', { count: minutes });
  const hours = Math.floor(minutes / 60);
  if (hours < 24) return i18n.t('time.ago_hours', { count: hours });
  const days = Math.floor(hours / 24);
  if (days < 30) return i18n.t('time.ago_days', { count: days });
