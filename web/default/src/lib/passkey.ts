...
/**
 * Server response types for Passkey operations.
 * The backend may return the credential options in different formats.
 */
export interface PasskeyServerResponse {
  publicKey?: ServerCredentialCreationOptions;
  PublicKey?: ServerCredentialCreationOptions;
  response?: ServerCredentialCreationOptions;
  Response?: ServerCredentialCreationOptions;
}

interface ServerCredentialCreationOptions {
  challenge: string;
  user?: {
    id?: string;
    name?: string;
...
export function arrayBufferToBase64Url(
  ...
  const encode =
    typeof globalRef.btoa === 'function'
      ...
      : (input: string) => {
          if (typeof globalRef.Buffer !== 'undefined') {
            return globalRef.Buffer.from(input, 'base64').toString('base64')
          }
          throw new Error(
...
export function prepareCredentialCreationOptions(
  ...
  } as unknown as PublicKeyCredentialCreationOptions & Record<string, unknown>

  if (Array.isArray(options.excludeCredentials)) {
    publicKey.excludeCredentials = options.excludeCredentials.map(
      (item: ServerCredentialDescriptor) => ({
        ...item,
  ...
  if (
    ...
    delete (publicKey as Record<string, unknown>).attestationFormats
  }
...
export function prepareCredentialRequestOptions(
  ...
  const publicKey: PublicKeyCredentialRequestOptions & Record<string, unknown> = {
    ...options,
    challenge: base64UrlToArrayBuffer(options.challenge),
  }
