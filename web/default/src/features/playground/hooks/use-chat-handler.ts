...
export function useChatHandler({
  ...
  // Handle stream error — strip HTML and show Chinese-friendly message
  const handleStreamError = useCallback(
    (error: string, errorCode?: string) => {
      // Strip raw HTML that may come from Nginx/Go error pages
      const cleanError = stripHtml(error) || error
      const displayError = cleanError.includes('<') || cleanError.length > 200
        ? '对话请求失败，请检查您的 API 令牌状态和账户余额。如持续遇到问题请稍后重试。'
        : cleanError
      toast.error(displayError)
      onMessageUpdate((prev) =>
        updateAssistantMessageWithError(prev, displayError, errorCode)
      )
    },
    [onMessageUpdate]
  )
  ...
  const sendNonStreamingChat = useCallback(
    async (messages: Message[]) => {
      ...
      const cleanRawErr = stripHtml(rawErr)
      handleStreamError(
        cleanRawErr.includes('<') || cleanRawErr.length > 200
          ? '对话请求失败，请检查您的 API 令牌状态和账户余额。'
          : (cleanRawErr || ERROR_MESSAGES.API_REQUEST_ERROR),
