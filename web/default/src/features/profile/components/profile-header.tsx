...
import {
  EditProfileDialog,
  ProfileEditButton,
} from './dialogs/edit-profile-dialog'
...
export function ProfileHeader({ profile, loading, onUpdate }: ProfileHeaderProps) {
          ...
          <div className='min-w-0 flex-1 space-y-1.5 sm:space-y-3'>
            <div className='flex min-w-0 items-center gap-2'>
              ...
              <StatusBadge
                ...
                copyable={false}
              />
              <ProfileEditButton onClick={() => setEditDialogOpen(true)} />
            </div>
            ...
            <div className='text-muted-foreground flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs sm:gap-x-4 sm:text-sm'>
              ...
              {profile.group && (
                <>
                  <span>•</span>
                  <span className='truncate'>{profile.group === 'default' ? t('默认') : profile.group}</span>
                </>
              )}
            </div>
          </div>
