...
import { Mail, Shield, Link2, Unlink, Smartphone } from 'lucide-react'
...
type DialogKey = 'email' | 'wechat' | 'phone'
  ...
  const bindings: BindingItem[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        isEnabled: Boolean(status?.wechat_login),
      ...
      },
      {
        ...
        label: t('OIDC'),
        icon: Shield,
        value: profile.oidc_id,
        isBound: Boolean(profile.oidc_id),
        isEnabled: Boolean(status?.oidc_enabled),
        onBind: () => {
      ...
      },
      {
        ...
        isEnabled: Boolean(status?.linuxdo_oauth),
        onBind: () => {
          if (status?.linuxdo_client_id) {
            handleLinuxDOOAuth(status.linuxdo_client_id as string)
          }
        },
      },
