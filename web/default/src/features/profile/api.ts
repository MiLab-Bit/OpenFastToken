 ...
 */
export async function sendPhoneCodeForBind(
  phone: string
): Promise<ApiResponse> {
  const res = await api.post('/api/phone/send_code', { phone, purpose: 'bind_phone' })
  return res.data
}
...
export async function bindPhone(
  ...
  const res = await api.post('/api/self/bind_phone', { phone, code })
  return res.data
}

/**
 * Unbind phone number from account
 */
export async function unbindPhone(
  code: string,
  password?: string
): Promise<ApiResponse> {
  const res = await api.post('/api/self/unbind_phone', { code, password })
  return res.data
}

/**
 * Get account bindings info (phone, email, wechat status)
 */
export async function getBindings(): Promise<ApiResponse<{
  bindings: Record<string, boolean>
  phone: string
  email: string
  wechat_bound: boolean
}>> {
  const res = await api.get('/api/self/bindings')
  return res.data
}
