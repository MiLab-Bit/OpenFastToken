...
export function resolveOllamaBaseUrl(channel: Channel | null) {
  ...
  const channelWithOllama = channel as Channel & { ollama_base_url?: string }
  const alt =
    typeof channelWithOllama.ollama_base_url === 'string'
      ? String(channelWithOllama.ollama_base_url).trim()
      : ''
  if (alt) return alt
