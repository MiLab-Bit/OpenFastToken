...
/**
 * Channel filter status. `''` means "all" and keeps the previous
 * `status?: string` semantics where an empty value selects every channel.
 */
...
export interface GetChannelsParams {
  p?: number
  page_size?: number
  status?: ChannelStatus // 'enabled' | 'disabled' | '' (empty = all)
  type?: number
  group?: string
  id_sort?: boolean
  tag_mode?: boolean
