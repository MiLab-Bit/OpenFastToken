...
function stripExprVersion(exprStr: string): { version: number; body: string } {
  // Callers (parseTiersFromExpr) already guard against empty input, so an
  // empty string simply yields an empty body here.
...
export function parseTiersFromExpr(exprStr: string): ParsedTier[] {
  if (!exprStr) return []
  // Neither stripExprVersion nor parseTierBody can throw (pure string/regex
  // work), so there is no recoverable failure path — parse directly.
...
function tryParseRuleGroupFactor(part: string): RequestRuleGroup | null {
  ...
  // A single condition may be serialized as a compound expression, e.g.
  // `param("m") != nil && has(param("m"), "v")` (CONTAINS) or
  // `param("n") != nil && param("n") >= 5` (numeric). Try the whole
  // condition string as one condition first; only fall back to splitting on
  // `&&` for genuinely multi-condition groups.
  const wholeCondition = tryParseRequestCondition(conditionStr)
  if (wholeCondition) {
    return { conditions: [wholeCondition], multiplier }
  }
  ...
  // `conditionStr` is guaranteed non-empty by the group-factor regex, and every
  // split part parses to a condition (otherwise we already returned null), so
  // `conditions` is always non-empty here.
...
function buildTimeConditionExpr(cond: TimeCondition): string {
  ...
  // `mode` is guaranteed to be one of MATCH_EQ/GTE/LT here (RANGE is handled
  // above and normalizeCondition only emits valid time modes), so no fallback.
