...
function applyRechargeRate(
  ...
  let result = (price * priceRate) / usdExchangeRate
  if (membershipDiscountRate !== 1) {
    result = result * membershipDiscountRate
  }
  return result
}
...
export function formatDynamicUnitPrice(
  ...
  const membershipDiscountRate = options.membershipDiscountRate ?? 1
  const priceUSD =
