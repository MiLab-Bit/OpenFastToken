...
import { RechargeGiftSettingsSection } from '../integrations/recharge-gift-settings-section'
...
const BILLING_SECTIONS = [
    ...
    build: (settings: BillingSettings) => (
      <PaymentSettingsSection
        defaultValues={{
          ...
          // amount_discount 已由后端废弃（僵尸配置），绝大多数情况下该 key 不存在；
          // 提供兜底空串，避免 formatJsonForEditor(undefined) 在打开支付设置页时抛错。
          ...
          AlipayUnitPrice: settings.AlipayUnitPrice ?? 1.0,
          ...
          WechatUnitPrice: settings.WechatUnitPrice ?? 1.0,
  ...
  {
    id: 'recharge-gift',
    titleKey: 'Recharge Gift',
    build: (settings: BillingSettings) => (
      <RechargeGiftSettingsSection
        defaultValues={{
          enabled: settings['recharge_gift_setting.enabled'] ?? false,
          tiers: settings['recharge_gift_setting.tiers'] || '[]',
          gift: settings['recharge_gift_setting.gift'] || '{}',
        }}
      />
    ),
  },
