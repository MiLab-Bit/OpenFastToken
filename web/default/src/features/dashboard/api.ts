...
import type { FlowResponse } from './lib/flow-selection'
...
export async function getFlowQuotaDates(
  ...
  const endpoint = isAdmin ? '/api/dashboard/flow/quota' : '/api/user/dashboard/flow/quota'
  const res = await api.get(endpoint, { params })
  return unsafeCast<FlowResponse>(res) ?? { success: true, data: [] }
}
