/*
Copyright (C) 2023-2026 FastToken

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU Affero General Public License as
published by the Free Software Foundation, either version 3 of the
License, or (at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU Affero General Public License for more details.

You should have received a copy of the GNU Affero General Public License
along with this program. If not, see <https://www.gnu.org/licenses/>.

For commercial licensing, please contact hello@fasttoken.example.com
*/
import { api } from '@/lib/api'
import type {
  ApiResponse,
  EnterpriseSubUser,
  GetSubUsersParams,
  GetSubUsersResponse,
  SearchSubUsersParams,
  CreateSubUserPayload,
  UpdateSubUserPayload,
} from './types'

// ============================================================================
// Enterprise Sub-User Management
// ============================================================================

// Get paginated sub-users list
export async function getSubUsers(
  params: GetSubUsersParams
): Promise<GetSubUsersResponse> {
  const { enterpriseId, p = 1, page_size = 10 } = params
  const res = await api.get(
    `/api/enterprise/${enterpriseId}/users?p=${p}&page_size=${page_size}`
  )
  return res.data
}

// Search sub-users by keyword
export async function searchSubUsers(
  params: SearchSubUsersParams
): Promise<GetSubUsersResponse> {
  const { enterpriseId, keyword = '', p = 1, page_size = 10 } = params
  const res = await api.get(
    `/api/enterprise/${enterpriseId}/users/search?keyword=${keyword}&p=${p}&page_size=${page_size}`
  )
  return res.data
}

// Get single sub-user by ID
export async function getSubUser(
  enterpriseId: number,
  id: number
): Promise<ApiResponse<EnterpriseSubUser>> {
  const res = await api.get(`/api/enterprise/${enterpriseId}/users/${id}`)
  return res.data
}

// Create sub-user
export async function createSubUser(
  enterpriseId: number,
  data: CreateSubUserPayload
): Promise<ApiResponse<EnterpriseSubUser>> {
  const res = await api.post(`/api/enterprise/${enterpriseId}/users`, data)
  return res.data
}

// Update sub-user
export async function updateSubUser(
  enterpriseId: number,
  data: UpdateSubUserPayload
): Promise<ApiResponse<EnterpriseSubUser>> {
  const res = await api.put(`/api/enterprise/${enterpriseId}/users`, data)
  return res.data
}

// Delete a single sub-user
export async function deleteSubUser(
  enterpriseId: number,
  id: number
): Promise<ApiResponse> {
  const res = await api.delete(`/api/enterprise/${enterpriseId}/users/${id}`)
  return res.data
}
