...
export function VendorDeleteDialog() {
  ...
  const { open, setOpen, currentRow, triggerRefresh } = useVendorContext()
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const result = await deleteVendor(currentRow.id)
      if (result.success) {
        toast.success(t(SUCCESS_MESSAGES.VENDOR_DELETED))
        setOpen(null)
        triggerRefresh()
      }
    } finally {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t('This will permanently delete vendor')}{' '}
