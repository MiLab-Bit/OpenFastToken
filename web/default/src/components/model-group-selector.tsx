...
export interface ModelGroupSelectorProps {
  ...
  groups: GroupOption[]
  onGroupChange: (value: string) => void
  // Admin control: if false, show read-only group badge instead of selector
  isAdmin?: boolean
  // Common props
  className?: string
  disabled?: boolean
...
export const ModelGroupSelector: React.FC<ModelGroupSelectorProps> = ({
  ...
  // For non-admin users: show read-only group badge instead of selectable dropdown
  if (!isAdmin) {
    ...
    return (
      <div className={cn('flex items-center gap-2', className)}>
        <div className='inline-flex items-center gap-1.5 rounded-full border bg-muted/50 px-3 py-1 text-xs'>
          ...
          <span className='font-medium'>{currentGroup?.label || t('Auto')}</span>
          {currentGroup?.ratio && typeof currentGroup.ratio === 'number' && (
            <span className='rounded-full bg-info/10 px-1.5 py-0.5 font-mono text-[10px] font-medium text-info tabular-nums'>
              {currentGroup.ratio}x
            </span>
          )}
        </div>
        <ModelSelector
