...
// 主题切换（浅色/深色/系统）已移除：皮肤选择器现在独占「主题」分区，
// 由 SkinProvider 驱动 <html data-skin>，明暗轴仍由 ThemeProvider 跟随系统。

function SkinConfig() {
  const { t } = useTranslation()
  const { skin, defaultSkin, skins, setSkin, resetSkin } = useSkin()
  return (
    <div>
      <SectionTitle
        title={t('Theme')}
      ...
      >
        {skins.map((s) => (
          <Item
            ...
            aria-label={t(`skin.${s.id}.name`, s.name)}
          >
            <div
              className={cn(
                'ring-border relative h-16 rounded-md ring-[1px] px-3 py-2 flex flex-col justify-center transition',
            ...
            >
              ...
              <span className='mt-0.5 truncate text-xs text-muted-foreground'>
                {t(`skin.${s.id}.description`, s.description)}
              </span>
            </div>
          </Item>
        ))}
