...
import { isRtlLang } from '@/lib/locale'
...
function RootComponent() {
  ...
  const [direction, setDirection] = useState(isRtlLang() ? 'rtl' : 'ltr')
  useEffect(() => {
    const update = () => setDirection(isRtlLang() ? 'rtl' : 'ltr')
    update()
...
export const Route = createRootRouteWithContext<{
  ...
  beforeLoad: async ({ location }) => {
    ...
    // 跳过 setup 状态检查（仅开发环境）
    const skipSetupCheck = import.meta.env.DEV

    // 只检查 setup 状态（如果需要）
    if (!skipSetupCheck && needsSetupCheck) {
    ...
    // Fix TanStack Router _nonReactive error: explicit return prevents
    // Promise.all from receiving undefined context
    return {}
  },
