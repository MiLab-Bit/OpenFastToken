...
func TestMain(m *testing.M) {
	f, err := os.CreateTemp("", "fasttoken_model_*.db")
	if err != nil {
...
func truncateTables(t *testing.T) {
	t.Helper()
	t.Cleanup(func() {
		taskDb.Exec("DELETE FROM tasks")
		taskDb.Exec("DELETE FROM users")
		taskDb.Exec("DELETE FROM tokens")
		taskDb.Exec("DELETE FROM logs")
		taskDb.Exec("DELETE FROM channels")
		taskDb.Exec("DELETE FROM abilities")
		taskDb.Exec("DELETE FROM top_ups")
		taskDb.Exec("DELETE FROM subscription_orders")
		taskDb.Exec("DELETE FROM subscription_plans")
		taskDb.Exec("DELETE FROM user_subscriptions")
		taskDb.Exec("DELETE FROM perf_metrics")
	})
}
...
func insertTask(t *testing.T, task *Task) {
	...
	require.NoError(t, taskDb.Create(task).Error)
}
