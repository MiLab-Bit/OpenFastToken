...
func TestGetPreferredModelOwnerChannelTypes(t *testing.T) {
	// Ensure the global DB points at the migrated test DB. Other model test
	// files (gift_test.go, topup_*_test.go) also assign model.DB; running the
	// full package in one binary means the last writer wins, so re-point here.
	DB = taskDb
	LOG_DB = taskDb
