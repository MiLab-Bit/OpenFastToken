...
// Disabled: goahocorasick functions - providing stubs
type stubMachine struct{}

func InitAc(dict []string) *stubMachine {
	fmt.Println("WARNING: goahocorasick is disabled, using stub")
	return &stubMachine{}
}
...
func getOrBuildAC(dict []string) *stubMachine {
	key := acKey(dict)
	if key == "" {
		return nil
	}
	if v, ok := acCache.Load(key); ok {
		if m, ok2 := v.(*stubMachine); ok2 {
			return m
		}
	}
	m := InitAc(dict)
	...
	if actual, loaded := acCache.LoadOrStore(key, m); loaded {
		if cached, ok := actual.(*stubMachine); ok {
			return cached
		}
	}
	return m
...
func (m *stubMachine) MultiPatternSearch(text []rune, stopImmediately bool) []stubHit {
	// Stub implementation - no actual search
	return nil
}
...
func AcSearch(findText string, dict []string, stopImmediately bool) (bool, []string) {
	...
	// Stub: always returns false (no match)
	return false, nil
}
