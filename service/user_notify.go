...
func NotifyRootUser(t string, subject string, content string) {
	user, err := model.GetRootUser()
	if err != nil {
		common.SysLog(fmt.Sprintf("failed to get root user: %s", err.Error()))
		return
	}
	userBase := user.ToBaseUser()
	err = NotifyUser(userBase.Id, userBase.Email, userBase.GetSetting(), dto.NewNotify(t, subject, content, nil))
	if err != nil {
...
func sendGotifyNotify(gotifyUrl string, gotifyToken string, priority int, data dto.Notify) error {
	...
	if system_setting.EnableWorker() {
		...
		req.Header.Set("User-Agent", "FastToken-Gotify-Notify/1.0")
