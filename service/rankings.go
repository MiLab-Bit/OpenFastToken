...
func buildRankingModelMeta(totals []model.RankingQuotaTotal) map[string]rankingModelMeta {
	...
	for _, pricing := range model.GetPricing() {
		...
		// 兜底：定价表里有记录但 vendor_id 为 0/未匹配时，按模型名推断厂商
		if item.vendor == rankingUnknownVendor || item.vendor == "" {
			if inferred, ok := model.InferDefaultVendor(pricing.ModelName); ok {
				item.vendor = inferred
				item.vendorIcon = model.InferDefaultVendorIcon(inferred)
			}
		}
		meta[pricing.ModelName] = item
	}

	// 处理有流量但不在定价表里的模型（例如已禁用、未配置价格的模型）
	for _, total := range totals {
		if _, exists := meta[total.ModelName]; exists {
			continue
		}
		item := rankingModelMeta{vendor: rankingUnknownVendor}
		if inferred, ok := model.InferDefaultVendor(total.ModelName); ok {
			item.vendor = inferred
			item.vendorIcon = model.InferDefaultVendorIcon(inferred)
		}
		meta[total.ModelName] = item
	}
