...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) {
	...
	if err := SettleBilling(ctx, relayInfo, summary.Quota); err != nil {
		// Refund pre-consumed quota so the user is not charged for an unsettled request
		if relayInfo.Billing != nil {
			relayInfo.Billing.Refund(ctx)
		}
		common.SysLog(fmt.Sprintf("CRITICAL: billing settlement failed user=%d model=%s quota=%d err=%s",
			relayInfo.UserId, summary.ModelName, summary.Quota, err.Error()))
	...
	// 异步写入消费日志（先捕获 gin.Context 值，防止 context 被回收）
	...
	otherCopy := make(map[string]interface{})
	for k, v := range other {
		otherCopy[k] = v
	}
	workerpool.Submit(func(ctx context.Context) {
		model.RecordConsumeLogAsync(username, requestId, upstreamRequestId, clientIP, userId, model.RecordConsumeLogParams{
			ChannelId:        channelId,
