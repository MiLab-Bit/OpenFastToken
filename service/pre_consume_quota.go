...
func PreConsumeQuota(c *gin.Context, preConsumedQuota int, relayInfo *relaycommon.RelayInfo) *types.FastTokenError {
	userQuota, err := model.GetUserQuota(relayInfo.UserId, false)
	if err != nil {
		return types.NewError(err, types.ErrorCodeQueryDataError, types.ErrOptionWithSkipRetry())
	}

	// Check if user belongs to an enterprise and has enterprise quota
	enterprise, entErr := model.GetUserEnterprise(relayInfo.UserId)
	if entErr == nil && enterprise != nil && enterprise.IsApproved() {
		eu, euErr := model.GetEnterpriseUser(enterprise.Id, relayInfo.UserId)
		if euErr == nil && eu != nil && eu.Status == "active" && eu.Quota > 0 {
			remainingEnterpriseQuota := eu.Quota - eu.UsedQuota
			if remainingEnterpriseQuota < preConsumedQuota {
				return types.NewErrorWithStatusCode(
					fmt.Errorf("企业子账号额度不足, 企业剩余额度: %s, 需要预扣费额度: %s",
						logger.FormatQuota(remainingEnterpriseQuota), logger.FormatQuota(preConsumedQuota)),
					types.ErrorCodeInsufficientUserQuota, http.StatusForbidden,
					types.ErrOptionWithSkipRetry(), types.ErrOptionWithNoRecordErrorLog(),
				)
			}
			// Enterprise sub-user quota is sufficient; proceed with user quota check below
			// The enterprise used_quota is incremented in PostConsumeQuota via SettleBilling
		}
	}
