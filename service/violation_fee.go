...
func HasCSAMViolationMarker(err *types.FastTokenError) bool {
	if err == nil {
		return false
	}
...
func WrapAsViolationFeeGrokCSAM(err *types.FastTokenError) *types.FastTokenError {
	if err == nil {
		return nil
	}
...
func NormalizeViolationFeeError(err *types.FastTokenError) *types.FastTokenError {
	if err == nil {
		return nil
	}
...
func shouldChargeViolationFee(err *types.FastTokenError) bool {
	if err == nil {
		return false
	}
...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.FastTokenError) bool {
