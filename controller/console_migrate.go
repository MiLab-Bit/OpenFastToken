...
func MigrateConsoleSetting(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": i18n.Msg(c, "获取配置失败，请稍后重试")})
		return
	}
	// 建立 map
	...
	c.JSON(http.StatusOK, gin.H{"success": true, "message": i18n.Msg(c, "migrated")})
}
