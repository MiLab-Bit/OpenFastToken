...
import (
	...
	"github.com/MiLab-Bit/OpenFastToken/setting/billing_setting"
...
func FetchUpstreamRatios(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		...
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": i18n.Msg(c, "请求参数格式错误")})
		return
	}
	...
	if len(req.Upstreams) > 0 {
		...
		if err != nil {
			...
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": i18n.Msg(c, "查询渠道失败")})
			return
		}
	...
	if len(upstreams) == 0 {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "无有效上游渠道")})
		return
	}
	...
	for _, chn := range upstreams {
		...
		go func(chItem dto.UpstreamDTO) {
			...
			if isOpenRouter && chItem.ID != 0 {
				dbCh, err := channelRepo().GetByID(chItem.ID, true)
				if err != nil {
...
func GetSyncableChannels(c *gin.Context) {
	channels, err := channelRepo().GetAll(0, 0, true, false)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": err.Error(),
