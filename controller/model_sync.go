...
// 上游地址
const (
	upstreamModelsURL  = "https://basellm.github.io/llm-metadata/api/FastToken/models.json"
	upstreamVendorsURL = "https://basellm.github.io/llm-metadata/api/FastToken/vendors.json"
)
...
func getUpstreamURLs(locale string) (modelsURL, vendorsURL string) {
	...
	if l, ok := normalizeLocale(locale); ok && l != "" {
		return fmt.Sprintf("%s/api/i18n/%s/FastToken/models.json", base, l),
			fmt.Sprintf("%s/api/i18n/%s/FastToken/vendors.json", base, l)
	}
	return fmt.Sprintf("%s/api/FastToken/models.json", base), fmt.Sprintf("%s/api/FastToken/vendors.json", base)
}
...
func SyncUpstreamModels(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "获取模型列表失败，请稍后重试")})
		return
	}
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "获取上游模型失败: ") + fetchErr.Error(), "locale": req.Locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	wg.Wait()
	if fetchErr != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "获取上游模型失败: ") + fetchErr.Error(), "locale": locale, "source_urls": gin.H{"models_url": modelsURL, "vendors_url": vendorsURL}})
		return
	}
