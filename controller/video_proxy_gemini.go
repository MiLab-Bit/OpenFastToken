...
func getGeminiVideoURL(channel *model.Channel, task *model.Task, apiKey string) (string, error) {
	...
	// Note: apiKey is passed separately and sent via HTTP header (x-goog-api-key)
	// in video_proxy.go. Do NOT append it to the URL to avoid leaking it in logs.
...
// ensureAPIKey is kept for backward compatibility with any callers that still
// need it, but new code should pass the key via HTTP header instead.
// Deprecated: use HTTP header (x-goog-api-key) instead of URL query parameters.
func ensureAPIKey(uri, key string) string {
	if key == "" || uri == "" {
		return uri
	}
	// Note: API keys in URL query parameters are logged by upstream servers
	// and intermediate proxies. Prefer HTTP headers for sensitive credentials.
