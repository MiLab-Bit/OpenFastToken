...
func startCodexOAuthWithChannelID(c *gin.Context, channelID int) {
	if channelID > 0 {
		ch, err := channelRepo().GetByID(channelID, false)
		if err != nil {
...
func completeCodexOAuthWithChannelID(c *gin.Context, channelID int) {
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "解析授权信息失败，请检查输入格式")})
		return
	}
	if strings.TrimSpace(code) == "" {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "missing authorization code")})
		return
	}
	if strings.TrimSpace(state) == "" {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "missing state in input")})
		return
	}
	...
	if strings.TrimSpace(expectedState) == "" || strings.TrimSpace(verifier) == "" {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "oauth flow not started or session expired")})
		return
	}
	if state != expectedState {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "state mismatch")})
		return
	}
	...
	if err != nil {
		...
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "授权码交换失败，请重试")})
		return
	}

	accountID, ok := service.ExtractCodexAccountIDFromJWT(tokenRes.AccessToken)
	if !ok {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": i18n.Msg(c, "failed to extract account_id from access_token")})
		return
	}
