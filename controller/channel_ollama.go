...
func OllamaPullModel(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": i18n.Msg(c, "Invalid request parameters"),
		})
		return
	}
	...
	if req.ChannelID == 0 || req.ModelName == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": i18n.Msg(c, "Channel ID and model name are required"),
		})
		return
	}

	// 获取渠道信息
	channel, err := channelRepo().GetByID(req.ChannelID, true)
	if err != nil {
		c.JSON(http.StatusNotFound, gin.H{
			"success": false,
			"message": i18n.Msg(c, "Channel not found"),
		})
		return
	}
	...
	if channel.Type != constant.ChannelTypeOllama {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": i18n.Msg(c, "This operation is only supported for Ollama channels"),
		})
		return
	}
...
func OllamaVersion(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"message": i18n.Msg(c, "Invalid channel id"),
		})
		return
	}
