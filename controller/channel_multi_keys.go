...
func ManageMultiKeys(c *gin.Context) {
	...
	channel, err := channelRepo().GetByID(request.ChannelId, true)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": i18n.Msg(c, "渠道不存在"),
		})
		return
	}
	...
	case "disable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": i18n.Msg(c, "未指定要禁用的密钥索引"),
			})
			return
		}
	...
	case "enable_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": i18n.Msg(c, "未指定要启用的密钥索引"),
			})
			return
		}
	...
	case "delete_key":
		if request.KeyIndex == nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": i18n.Msg(c, "未指定要删除的密钥索引"),
			})
			return
		}
	...
	case "delete_disabled_keys":
		...
		if deletedCount == 0 {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": i18n.Msg(c, "没有需要删除的自动禁用密钥"),
			})
			return
		}
