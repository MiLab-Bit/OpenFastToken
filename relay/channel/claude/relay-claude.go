...
import (
	...
	"www.abc-ai.cn/FastToken/relay/channel/openrouter"
	relaycommon "www.abc-ai.cn/FastToken/relay/common"
	"www.abc-ai.cn/FastToken/relay/helper"
	"www.abc-ai.cn/FastToken/relay/reasonmap"
	"www.abc-ai.cn/FastToken/service"
	"www.abc-ai.cn/FastToken/setting/model_setting"
	"www.abc-ai.cn/FastToken/setting/reasoning"
	...
	for _, message := range formatMessages {
		if message.Role == "system" {
			...
			if message.Role == "tool" {
				...
				default:
					// OpenAI file content: convert by the underlying file type inferred
					// from the filename extension (txt -> inline text, pdf -> document,
					// image -> image, others ignored).
					if mediaMessage.Type == dto.ContentTypeFile {
						file := mediaMessage.GetFile()
						if file == nil || file.FileData == "" {
							continue
						}
						mediaType := inferFileMediaType(file.FileName)
						switch {
						case mediaType == "text/plain":
							decoded, derr := base64.StdEncoding.DecodeString(file.FileData)
							if derr != nil {
								decoded = []byte(file.FileData)
							}
							claudeMediaMessages = append(claudeMediaMessages, dto.ClaudeMediaMessage{
								Type: "text",
...
func HandleStreamResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, data string) *types.FastTokenError {
...
func ClaudeStreamHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.FastTokenError) {
...
func HandleClaudeResponseData(c *gin.Context, info *relaycommon.RelayInfo, claudeInfo *ClaudeResponseInfo, httpResp *http.Response, data []byte) *types.FastTokenError {
	var claudeResponse dto.ClaudeResponse
...
func ClaudeHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo) (*dto.Usage, *types.FastTokenError) {
...
// inferFileMediaType returns the MIME type for a file based on its filename
// extension, used when converting OpenAI file content to Claude messages.
// Unknown extensions fall back to application/octet-stream (treated as
// unsupported and ignored by the caller).
func inferFileMediaType(fileName string) string {
