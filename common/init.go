...
func printHelp() {
	...
	fmt.Println("Homepage: https://www.abc-ai.cn")
...
func InitEnv() {
	...
	RelayTimeout = GetEnvOrDefault("RELAY_TIMEOUT", 300) // 默认 5 分钟超时，防止请求无限挂起
	// 验证 RelayTimeout 范围
	if RelayTimeout < 10 {
		log.Println("WARNING: RELAY_TIMEOUT is too small, setting to minimum 10 seconds")
		RelayTimeout = 10
	} else if RelayTimeout > 3600 {
		log.Println("WARNING: RELAY_TIMEOUT is too large, setting to maximum 3600 seconds (1 hour)")
		RelayTimeout = 3600
	}
