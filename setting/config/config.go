...
// unmarshalLoose 反序列化 JSON；严格解析失败时尝试宽松修复（为未加引号的键补引号），
// 并在最终失败时输出告警日志，避免配置字段被静默丢弃导致功能“没走”。
func unmarshalLoose(dst interface{}, raw, key string) error {
    err := json.Unmarshal([]byte(raw), dst)
    if err == nil {
        return nil
    }
    fixed := looseJSONKeys(raw)
    if fixed != raw {
        err2 := json.Unmarshal([]byte(fixed), dst)
        if err2 == nil {
            log.Printf("[config] WARN: option %q 含非法 JSON（未加引号的键），已自动修复并加载", key)
            return nil
        }
        log.Printf("[config] ERROR: option %q JSON 解析失败（宽松修复也失败）: %v | raw=%q", key, err2, raw)
        return err2
    }
    log.Printf("[config] ERROR: option %q JSON 解析失败: %v | raw=%q", key, err, raw)
    return err
}

// looseJSONKeys 将 JS 风格对象字面量（键未加引号）转为合法 JSON，
// 例如 {amount:12} -> {"amount":12}。已加引号的键不受影响。
func looseJSONKeys(s string) string {
	...
	for i := 0; i < val.NumField(); i++ {
		...
		case reflect.Ptr:
			...
			if strValue == "null" {
				...
				// 反序列化到指针指向的值
				if err := unmarshalLoose(field.Interface(), strValue, key); err != nil {
				continue
			}
			}
		case reflect.Map:
			...
			if err := unmarshalLoose(fresh.Interface(), strValue, key); err != nil {
				continue
			}
			field.Set(fresh.Elem())
		case reflect.Slice, reflect.Struct:
			if err := unmarshalLoose(field.Addr().Interface(), strValue, key); err != nil {
				continue
			}
		}
	}
